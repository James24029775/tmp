...
void Socks4Server::run()
{
    _acceptor.async_accept(_context, [this](auto ec, tcp::socket socket)
        ...
        if (!ec)
        {
            ...
            auto key = ep.address().to_string();
            if (ipExist.find(key) == ipExist.end()) // doesn't exist
            {
                pid = fork();
                if (pid == -1)
                {
                    ...
                    while (true)
                    {
                        ...
                        double elapsed_time = std::difftime(current_time, start_time);
...
{
    auto browsorEndpoint = _browsorSocket.remote_endpoint();
    int elapsed_time = 0;
    auto key = browsorEndpoint.address().to_string();
    ...
    if (std::getline(infile, line)) 
    {
        ...
        double e;
        if (iss >> e) 
            elapsed_time = static_cast<int>(e);
    }
    if (elapsed_time >= 10){
        std::cout << "<Reply>: Reject" << std::endl;
        reject();
